...
\IfFileExists{ciad-utbmgraylogo.pdf}{%
	\pgfdeclareimage[height=.2cm]{fakeutbmlogoinpartners}{ciad-utbmgraylogo}%
}{%
	\pgfdeclareimage[height=.2cm]{fakeutbmlogoinpartners}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-ubegraylogo.pdf}{%
	\pgfdeclareimage[height=.2cm]{fakeubelogoinpartners}{ciad-ubegraylogo}%
}{%
	\pgfdeclareimage[height=.2cm]{fakeubelogoinpartners}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-carnotgraylogo.pdf}{%
	\pgfdeclareimage[height=.2cm]{fakecarnotlogoinpartners}{ciad-carnotgraylogo}%
}{%
	\pgfdeclareimage[height=.2cm]{fakecarnotlogoinpartners}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-logointitle.pdf}{%
	\pgfdeclareimage[height=.25cm]{fakeciadlogointitle}{ciad-logointitle}%
}{%
	\pgfdeclareimage[height=.25cm]{fakeciadlogointitle}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-utbmlogointitle.pdf}{%
	\pgfdeclareimage[height=.25cm]{fakeutbmlogointitle}{ciad-utbmlogointitle}%
}{%
	\pgfdeclareimage[height=.25cm]{fakeutbmlogointitle}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-ubelogointitle.pdf}{%
	\pgfdeclareimage[height=.25cm]{fakeubelogointitle}{ciad-ubelogointitle}%
}{%
	\pgfdeclareimage[height=.25cm]{fakeubelogointitle}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-utbmlogo.pdf}{%
	\pgfdeclareimage[height=.25cm]{fakeuserlogointitle}{ciad-utbmlogo}%
}{%
	\pgfdeclareimage[height=.25cm]{fakeuserlogointitle}{ciadbeamer-nologo}%
}
...
\begin{frame}{Read the Beamer Documentation}
	\huge
	\alertbox{It is recommended to read the user guide of Beamer for obtain a view of the available commands.}
	\vspace{2em}
...
\begin{frame}[t]{Class Options}
	...
	\begin{block}{Automatic slides}
		...
		\item \texttt{conferencepresentation}: Use the "Conference Presentation" front page instead of the regular title page.
...
\begin{frame}[t]{Class Options \insertcontinuationtext}
	\begin{block}{Footline and Headline}
		...
		\item \texttt{carnotlogo} and \texttt{nocarnotlogo}: enable or disable the logo of "Institut Carnot ARTS" in the foot line.
...
\begin{frame}[t]{Class Options \insertcontinuationtext}
	...
	\begin{block}{Handout --- Free space beside slides for hand notes}
		...
		\end{itemize}
	\end{block}
	\begin{block}{Bibliography --- DOIs or regular labels}
		\begin{itemize}
		\item \texttt{doiinbib}: the labels of the bibliography references are DOI if defined, otherwise the regular bibliography label is used (see \texttt{nodoiinbib} option).
		\item \texttt{nodoiinbib}: the labels of the bibliography references are based on the names and year of publication, e.g., \texttt{[Galland, 2025]}.
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{Title and Subtitle}
	...
	\end{block}
	\vfill
	\alertbox{These commands must be put in the preamble of your document.}
\end{frame}
...
\begin{frame}[t]{Change the Right Picture \textcircled{2}}
	...
	\end{itemize}
	\begin{example}
		\texttt{{\textbackslash}pgfdeclareimage[width=5cm]\{mypicture\}\{ciad-utbmlogo\}} \\
...
\begin{frame}[t]{Change the Left Background \textcircled{3}}
	...
	\end{itemize}
	\alertbox{The left picture is automatically removed when the command above is invoked}
	\begin{example}
...
\begin{frame}{Author Description}
	...
	\end{itemize}
	\vfill
	\alertbox{This command must be put in the preamble of your document.}
\end{frame}
...
\begin{frame}{{Title Page} for PhD Oral Defense}
	\begin{itemize}
	\item It is possible to change the regular title page by a title page dedicated to \Emph{PhD or Master oral defense}.
	...
	\vfill
	\centering
	\framebox{\includegraphics[width=.5\linewidth]{exampleodtp}}
\end{frame}
...
\begin{frame}{{Committee Members} on the Title Page for PhD Oral Defense}
	\alertbox*{The members of the oral defense committee could be set with the following command}
	\begin{itemize}
...
\subsection{Title Page for Conference Presentation}

\begin{frame}{{Title Page} for Conference Presentation}
	\begin{itemize}
	\item It is possible to change the regular title page by a title page dedicated to \Emph{Conference Presentations}.
	\item For enabling the page, use the class option "\texttt{conferencepresentation}".
	\item The configuration of the content of this special front page is explained in the following slides
	\end{itemize}
	\vfill
	\centering
	\framebox{\includegraphics[width=.5\linewidth]{examplecptp}}
\end{frame}

\begin{frame}{{Configuration} of the Title Page for Conference Presentation}
	\begin{itemize}
	\item Setting the title, the authors and the event is explained in the other sections
	\item \texttt{{\textbackslash}affilitation\{<id>\}\{<name>\}} add an affilitation with the given identifier
	\item \texttt{{\textbackslash}affil\{<id>\}} show affilitation identifier, usually on the side of the author names
	\item \texttt{{\textbackslash}resetaffiliations} reset the list of affiliations
	\item \texttt{{\textbackslash}date\{<text>\}} changes the conference presentation
	\item \texttt{{\textbackslash}place\{<text>\}} changes the place/location of the conference
	\item \texttt{{\textbackslash}addpresentationlogo\{<image path>\}} adds a logo at the bottom of the page
	\item \texttt{{\textbackslash}resetpresentationlogo} removes all the logo from the front page
	\item \texttt{{\textbackslash}resetpresentationlogotodefault} removes all the logo from the front page but keep the CIAD laboratory logos
	\item \texttt{{\textbackslash}setpresentationbackground(<x>,<y>)\{<background index>\}} changes the background figure. Index is the number of the background image as defined for the regular slides. x and y are the coordinates of the images on the front page
	\end{itemize}
\end{frame}
...
\begin{frame}{Institute URL}
	...
	\vspace{1em}
	\item To insert the institute's URL, you could use the command: \\
...
\begin{frame}{Headline}
	...
	\item You could change this headline with: \begin{itemize}\footnotesize
		...
		\item \texttt{{\textbackslash}useheaderdefault}: the headline is filled with the default value (may be changed with one of the commands on the following slide).
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{\subsecname}
	...
	\vfill
	\item \inlineexample{in the following frame, the command is used in the title \texttt{{\textbackslash}insertcontinuationtext}.}
	\item \inlineexample{in the second following frame, the command is used in the title \texttt{{\textbackslash}insertcontinuationwith\{34\}}.}
	\end{itemize}
\end{frame}
...
\begin{frame}[t]{{Pre-defined Background Picture:} 1}
	...
	\alertbox{If no picture appears at background, it means that you have not installed the non-free background pictures}
\end{frame}
...
\begin{frame}{Table of Contents/Outline}
	...
	\item \texttt{{\textbackslash}outlinetitlelabel} is the default title of the outline slide.
	\end{itemize}
\end{frame}
...
\begin{frame}{Part Sectioning}
	...
	\item Options may be: \begin{itemize}
		...
		\item the pair \texttt{subtitle=text} to define the subtitle.
...
\subsubsection{Bibliography with regular \LaTeX\ macros}

\begin{frame}{Regular \LaTeX\ Bibliography}
	\begin{itemize}
...
\subsubsection{Bibliography with Bibunits}

\begin{frame}{Bibliography with Bibunits}
	\begin{itemize}
	\item \texttt{Bibunits} package allows to create separate, individual bibliographies for specific parts
	\item To use \texttt{Bibunits}, you have to: \\
		\texttt{{\textbackslash}usepackage\{bibunits\}}
	\item The two following elements are automatically activated when you include \texttt{Bibunits}:
		\begin{enumerate}
		\item \texttt{{\textbackslash}bibliographyslide} create a frame with the bibliography in the current \texttt{Bibunits} section and using the regular \texttt{Bibunits} macro: \texttt{{\textbackslash}putbib[biblio]}. \\
			Please note that the name of the Bib\TeX\ database is always \texttt{biblio.bib}.
		\item The following environment permits to create a specific \texttt{Bibunits} block: \\
			\texttt{{\textbackslash}begin\{bibliographysection\}} \\
			... \\
			\texttt{{\textbackslash}end\{bibliographysection\}} \\
			This environment automatically expands to \texttt{{\textbackslash}bibliographyslide} at the end of the section block.
		\end{enumerate}
	\end{itemize}
\end{frame}
...
\subsection{Slide with a title on the right}
\subsectiontableofcontentslide
...
\subsection{Slide with text and a picture}
\subsectiontableofcontentslide
...
\subsection{Slide with lawn (vertical green area)}
\subsectiontableofcontentslide
...
\subsection{Slide with a single figure on the center}
\subsectiontableofcontentslide

\begin{frame}{Slide with a Single Figure}
	\begin{itemize}
	\item The CIAD theme provides a command that permits to display a picture on the entire slide: \\
	...
	\item Example: \texttt{{\textbackslash}figureslide\{XyZ\}\{ciad-ciadlogo\}}
	\item If no image appears below, it is because you have not installed the non-free logos
	\end{itemize}
	\vspace{1em}
	\begin{center}
		\fakeslide{XyZ}{
			\IfFileExists{ciad-ciadlogo.pdf}{
				\put(-3,20){\includegraphics[width=\linewidth]{ciad-ciadlogo}}
			}{
				\put(-3,20){\includegraphics[width=\linewidth]{ciadbeamer-nologo}}
			}
		}
	\end{center}
\end{frame}
...
\IfFileExists{ciad-ciadlogo.pdf}{%
	\figureslide{Example of a Single Figure on a Slide}{ciad-ciadlogo}
}{}

\subsection{Slide with an ``animated'' figure on the center}
\subsectiontableofcontentslide

\begin{frame}{How to Create an ``Animated'' Figure?}
	\smaller
	\alertbox{You must use the AutoLaTeX\footnote{\url{http://www.arakhne.org/autolatex}} tool also included in many \TeX\ distributions (e.g., {\TeX}Live.}
	\begin{enumerate}
	...
	\item Save and run AutoLaTeX. This tool will create a PDF file for each layer, and a \TeX\ file (\texttt{.pdftex\string_t}) that is controlling the displaying of the figure.
...
\begin{frame}{Example of ``Animated'' Figure with \texttt{{\textbackslash}input}}
	\begin{itemize}
	\item Content of the figure: Three layers are defined with the following names:
		\begin{itemize}
		...
		\end{itemize}
	\item Inclusion method: \texttt{{\textbackslash}input\{path-to-figure.pdftex\string_t\}}
	\end{itemize}
...
\begin{frame}{Exemple of inclusion code with \texttt{{\textbackslash}input} (used on the previous slide)}
	\texttt{{\textbackslash}resizebox\{.3{\textbackslash}linewidth\}\{!\}\{} \\
	\texttt{\hspace{.5cm}{\textbackslash}resolvepicturename\{image-name\}} \\
	\texttt{\hspace{.5cm}{\textbackslash}input\{{\textbackslash}resolvedfilename\}} \\
	\texttt{\}}
\end{frame}

\begin{frame}{Exemple of inclusion code with macros from AutoLaTeX}
	\texttt{{\textbackslash}usepackage\{autolatex\}} \\
	\vspace{.5cm}
	\texttt{...} \\
	\vspace{.5cm}
	\texttt{{\textbackslash}resolvepicturename\{image-name\}} \\
	\texttt{{\textbackslash}includeanimatedfigure[width=.3{\textbackslash}linewidth]\{{\textbackslash}resolvedfilename\}}
\end{frame}
...
\begin{frame}{Slide with a Single ``Animated'' Figure}
	...
	\end{itemize}
	\vspace{.5cm}
	\texttt{{\textbackslash}usepackage\{autolatex-beamer\}} \\
	\vspace{.25cm}
	\texttt{...} \\
	\vspace{.25cm}
	\texttt{{\textbackslash}animatedfigureslide\{Some title\}\{image-name\}}
\end{frame}

\subsection{Slide with a single video on the center}
\subsectiontableofcontentslide
...
\begin{frame}{Slide with a Single Embedded Video}
	...
	\item The size of the picture is adjusted to the slide drawing area.
	\item Example: \texttt{{\textbackslash}embeddedvideoslide\{XyZ\}\{myvideo\}\{ciad-ciadlogo\}} 
...
\subsection{Slide with text, video and logos}
\subsectiontableofcontentslide

\begin{frame}{Slide with Text, Video and Logos}
	\begin{itemize}
		\item Slide with a regular text area on the left, a video inside a big green area on the right and logos below the video.
		\item Definition with the following environment: \\
		\texttt{{\textbackslash}begin\{textvideoframe\}[options]\{main title\}\{video-path\}\{picture-path\}} \\
		\texttt{Content of the Slide} \\
		\texttt{{\textbackslash}end\{textvideoframe\}}
		\item \texttt{options} are options to pass to the frame
		\item \texttt{main title} is the title to be rendered at the top of the frame
		\item \texttt{video-path} is the path to the video
		\item \texttt{picture-path} is the path to the picture that is rendered in place of the video
		\item \texttt{{\textbackslash}settextvideofooter\{text\}} permits to set the logo text below the video
	\end{itemize}
\end{frame}

\settextvideofooter{XYZ}
\begin{textvideoframe}{{Example of frame} with left text and right video}{fakevideo.mp4}{examplecptp}
	This is the standard frame content. See the previous slide for an explanation \\[.5cm]
	\texttt{{\textbackslash}settextvideofooter\{XYZ\}} \\[.5cm]
	\texttt{{\textbackslash}begin\{textvideoframe\}} \\
	\texttt{~~\{Example of frame[...]\}} \\
	\texttt{~~\{fakevideo.mp4\}\{examplecptp\}} \\
	\texttt{~~This is [...]} \\
	\texttt{{\textbackslash}end\{textvideoframe\}}
\end{textvideoframe}


\subsection{Final or conclusion slide}
\subsectiontableofcontentslide
...
\subsection{Slide with book description}
\subsectiontableofcontentslide
...
\begin{frame}{{\textbackslash}includegraphics}
	\alertbox{The command \textbf{{\textbackslash}includegraphics} is overridden by the theme.}
	\begin{itemize}
	\vspace{2em}
...
\begin{frame}{Compact Itemized List}
	The CIAD theme provides a compact version of the itemized list environment. \\[.5cm]
	{\smaller
	\texttt{{\textbackslash}begin\{compactitemize\}[options]} \\
	\texttt{{\textbackslash}item text1} \\
	\texttt{{\textbackslash}item$<$frame\_spec$>$ text2} \\
	\texttt{{\textbackslash}end\{compactitemize\}}} \\[.5cm]
	\begin{columns}
		\begin{column}{.5\linewidth}
			\begin{block}{Compact List}
...
\begin{frame}{Compact Description}
	The CIAD theme provides a compact version of the description environment. \\[.5cm]
	{\smaller
	\texttt{{\textbackslash}begin\{compactdescription\}} \\
	\texttt{{\textbackslash}item text1} \\
	\texttt{{\textbackslash}item[Item Name] text2} \\
	\texttt{{\textbackslash}item$<$frame\_spec$>$ text1} \\
	\texttt{{\textbackslash}item$<$frame\_spec$>$[Item Name] text2} \\
	\texttt{{\textbackslash}end\{compactdescription\}}} \\[.5cm]
	\begin{columns}
		\begin{column}{.5\linewidth}
			\begin{block}{Compact Description}
		...
		\end{column}
		\begin{column}{.5\linewidth}
			\begin{block}{Standard Description}
				\begin{description}
				\item text1
				\item[Item Name] text2
				\item<2> text1
...
\begin{frame}{{Wrapped Figure} on Side of Itemizes}
	...
	\end{itemize}
	\begin{example}\tiny
		\wrapfigure[width=.1\linewidth]{examplecptp}
		\begin{itemize}
...
\begin{frame}[t]{Link to a Video}
	...
	\end{itemize}
	\begin{center}
		\videolink[width=.3\linewidth]{./ciadbeamer-documentation.tex}{examplecptp}
	\end{center}
\end{frame}
...
\begin{frame}[t]{Link to a Slide with an Picture}
	...
	\end{itemize}
	\begin{center}
		\picturegoto[width=.3\linewidth]{progressbartypes}{examplecptp}
	\end{center}
\end{frame}
...
\begin{frame}{Link to a Web Page with an Text}
	\begin{itemize}
	\item The CIAD theme provides a convenient command to create links to Web pages with a text in the link.
	\item \texttt{{\textbackslash}hrefbutton[text]\{url\}}
		\begin{itemize}
		\item Display the given text (default is \texttt{web}) with a link button
		\item \texttt{url} is the URL of the target Web pge.
		\item \texttt{text} is the text to display in the slide.
		\end{itemize}
	\end{itemize}
	\begin{center}
		\hrefbutton{https://www.ciad-lab.fr}
	\end{center}
\end{frame}
...
\IfFileExists{ciad-ciadlogo.pdf}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalciadlogo}{ciad-ciadlogo}%
}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalciadlogo}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-utbmlogo.pdf}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalutbmlogo}{ciad-utbmlogo}%
}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalutbmlogo}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-ubelogo.pdf}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalubelogo}{ciad-ubelogo}%
}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalubelogo}{ciadbeamer-nologo}%
}
\IfFileExists{ciad-carnotlogo.pdf}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalcarnotlogo}{ciad-carnotlogo}%
}{%
	\pgfdeclareimage[width=.5\linewidth]{transitionalcarnotlogo}{ciadbeamer-nologo}%
}
...
\begin{frame}{Multilingual Support}
	Talks may be written with frames in different languages. The CIAD Beamer template provides commands for helping to building multilingual documents.
	\begin{itemize}
	\item \texttt{{\textbackslash}languageshortname}: command that is replaced by the short name of the current language (\texttt{en} for English, \texttt{fr} for French, \texttt{ge} for German)
	\item \texttt{{\textbackslash}inputlang\{filename\}}: similar to the standard \texttt{{\textbackslash}input} but searching for a file marked for the current language with the name \texttt{filename\_LG} where \texttt{filename} is the argument and \texttt{LG} is the current language code given by \texttt{{\textbackslash}languageshortname}. If the file with name \texttt{filename\_LG} is not found, then the file with name \texttt{filename} is input.
	\item \texttt{{\textbackslash}includelang\{filename\}}: similar to the standard \texttt{{\textbackslash}include} but searching for a file marked for the current language with the name \texttt{filename\_LG} where \texttt{filename} is the argument and \texttt{LG} is the current language code given by \texttt{{\textbackslash}languageshortname}. If the file with name \texttt{filename\_LG} is not found, then the file with name \texttt{filename} is included.
	\end{itemize}
\end{frame}

\begin{frame}{Use Localized Commands}
	\begin{itemize}
	\item Several commands are implemented with the search method described in the previous slide:
		\begin{stabularx}[title={List of Localized Commands}]{XX}
			\tabularhead{Command}{Localized Version} \\
			\texttt{{\textbackslash}input} & \texttt{{\textbackslash}inputlang} \\
			\texttt{{\textbackslash}include} & \texttt{{\textbackslash}includelang} \\
			\texttt{{\textbackslash}figureslide} & \texttt{{\textbackslash}figureslide} \\
		\end{stabularx}
	\item Command \texttt{{\textbackslash}localized} permits to use the localized version of a known command
	\end{itemize}
	\begin{block}{Example}
		\texttt{{\textbackslash}localized{\textbackslash}input\{myfile\}} is equivalent to \texttt{{\textbackslash}inputlang\{myfile\}}
	\end{block}
\end{frame}
